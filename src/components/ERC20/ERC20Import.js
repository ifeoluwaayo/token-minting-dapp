...
const ERC20Import = ({ tokenAddress }) => {
	...
	useEffect(() => {
		async function fetchData() {
			const web3Token = new web3.eth.Contract(
				ERC20Token.abi,
				tokenAddress
			);
	...
	return (
		<div className="p-4">
			<Grid container spacing={2}>
				...
				<Grid item xs={12} sx={{ height: 500, p: 2 }} className="">
					<DataGrid
						rows={tokenData}
						columns={columns}
						className="px-4"
			...
			<Box
				...
				borderColor="grey.200"
				sx={{ m: 2, borderRadius: 1 }}>
				<TransferFrom web3Token={web3Token} tokenData={tokenData} />
			</Box>
			<Box
				border={1}
				borderColor="grey.200"
				sx={{ m: 2, borderRadius: 1 }}>
				<Approve web3Token={web3Token} tokenData={tokenData} />
			</Box>
		</div>
	);
};
